...
class data_loader:
    ...
    def load_nodes(self):
        # 'name':{} is used to store natural language descriptions of all nodes (such as paper titles, author names, etc.)
        nodes = {'total':0, 'count':Counter(), 'attr':{}, 'shift':{}, 'name':{}}
        with open(os.path.join(self.path, 'node.dat'), 'r', encoding='utf-8') as f:
            for line in f:
                ...
                if len(th) == 4:
                    ...
                    nodes['name'][node_id] = node_name.strip() 
                    nodes['total'] += 1
                elif len(th) == 3:
